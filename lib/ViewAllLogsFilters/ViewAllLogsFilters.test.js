...
const renderViewAllLogsFilters = (props = {}) => {
  render(
    <Router>
      <ViewAllLogsFilters
...
describe('ViewAllLogsFilters', () => {
  ...
  it('should render user lookup insted of users select if "showUsers" - false', () => {
    renderViewAllLogsFilters({ showUsers: false });

    expect(screen.getByText('stripes-data-transfer-components.filter.user')).toBeVisible();
  });
});
