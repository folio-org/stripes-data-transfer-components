...
import { DEFAULT_JOB_LOGS_SORT_COLUMNS } from './columnProperties';
...
const withJobLogsSortComponent = WrappedComponent => {
  return class extends Component {
    ...
    static defaultProps = {
      formatter: {},
      sortColumns: DEFAULT_JOB_LOGS_SORT_COLUMNS,
      contentData: [],
      hasLoaded: false,
    };
    ...
    prepareLogsData() {
      ...
      const logs = contentData.map(log => ({
        ...log,
        jobProfileName: log.hasOwnProperty('jobProfileName') ? log.jobProfileName: log.jobProfileInfo.name,
      }));
