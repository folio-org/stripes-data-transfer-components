...
describe('MappingProfiles', () => {
  ...
  describe('rendering MappingProfiles with empty profiles list', () => {
    ...
    beforeEach(async () => {
      await mountWithContext(<MappingProfileContainer parentResources={parentResources} />);
    });
  ...
  describe('rendering MappingProfiles with filled profiles list and default list properties', () => {
    ...
    it('should place only default columns in correct order', () => {
      ...
      expect(mappingProfiles.searchResults.list.headers(0).text).to.equal(translations.name);
      expect(mappingProfiles.searchResults.list.columnCount).to.equal(4);
    });
  });
  ...
  describe('rendering MappingProfiles with filled profiles list and custom field', () => {
    ...
    it('should place headers in correct order', () => {
      ...
      expect(mappingProfiles.searchResults.list.columnCount).to.equal(5);
    });
